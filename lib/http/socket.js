...
let socketService, sqlite3;
const io = require('socket.io')();
const EventEmitter = require('events');
class MyEmitter extends EventEmitter { }

exports.createServer = async (options) => {
    let sqlite3file = options.db || options.dbpath || options.sqlite3Path;
    if (sqlite3file) {
        fac.fs.mkdirSync(sqlite3file);
        sqlite3file = path.join(sqlite3file, fac.global().sqlite3DbName);
    }
    sqlite3 = new fac.sqlite3(sqlite3file || fac.global().sqlite3DbName, false);
