...
exports.createServer = async (options) => {
    sqlite3 = new fac.sqlite3(options.sqlite3file, false);
    ...
    socketService.send = function (clientid, event, ...message) {
        ...
        sqlite3.run(`\
            insert into socket_log(\
                ...
                '${fac.guid.v22}','[SERVER]','${clientid || ""}','${fac.process.id}',?,?,'${new Date().format()}' \
            );\
        `, [
            event || "",
            message || ""
    ...
    socketService.broadcast = function (event, ...message) {
        ...
        sqlite3.run(`\
            insert into socket_log(\
                ...
                '${fac.guid.v22}','[SERVER]','[CLIENT]','${fac.process.id}',?,?,'${new Date().format()}' \
            );\
        `, [
            event || "",
            message || ""
    ...
    if (client.id) {
        ...
        sqlite3.run(`\
            insert into socket_log(\
                ...
                '${_log.id}','${_log.from || ""}','${_log.to || ""}','${_log.pid}',?,?,'${new Date().format()}' \
            );\
        `, [
            _log.url || "",
            _log.body || ""
