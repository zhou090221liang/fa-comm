...
server = http.createServer(async (req, res) => {
    try {
        ...
        await sqlite3.run(`\
            insert into api_log(\
                id,host,qid,pid,ip,url,path,method,query,params,body,headers,req_time\
            )values(\
                '${req.id}','${req.host}','${req.qid}','${req.pid}','${req.ip}','${req.url}','${req.path}',\
                '${req.method}',?,?,?,?,'${new Date().format()}'\
            );\
        `, [
        ...
        for (const router in routers[req.method.toLowerCase()]) {
            if (router == req.path) {
                ...
                if (middlewareResult) {
                    ...
                    await sqlite3.run(`\
                        update api_log set res_status = '200',res_message = ?,\
            ...
            if (_router.length == _path.length) {
                ...
                if (params) {
                    ...
                    // }, false);
                    await sqlite3.run(`\
                        update api_log set params = ? where id = '${req.id}';\
                    `, [(req.params ? JSON.stringify(req.params) : "")], false);
                    const middlewareResult = await runMiddleware(req);
                    if (middlewareResult) {
                        // await facomm_apilog.update({
                        //     id: req.id,
        ...
        if (req.id) {
            ...
            await sqlite3.run(`\
                update api_log set res_status = '500',res_message = ?,\
                res_time = '${new Date().format()}' where id = '${req.id}';\
            `, [JSON.stringify(ERRMESSAGE)], false);
        }
        res.writeHead(500);
        res.end(e.message);
    }
