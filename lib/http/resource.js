                                ...
                                if (files[key].name && files[key].size && files[key].path && files[key].type && files[key].lastModifiedDate && files[key].hash) {
                                    ...
                                    if (exists && exists.length) {
                                        if (fs.existsSync(exists[0].path + exists[0].file_name)) {
                                            ...
                                            await sqlite3.run(`update upload set file_name = ?,use_time = ?,speed = ? where md5 = '${files[key].md5}'`, [files[key].file_name, now, fac.fs.formatSize(speed)], false);
                                        }
                                    }
                                    // const _result = await facomm_upload.insert(files[key]);
                                    const _result = await sqlite3.run(`\
                                        insert into upload(\
                                            id,size,form,origin_name,type,boundary,file_name,path,md5,batch,upload_time,extend,use_time,speed\
                                        )values(\
                                            ?,?,?,?,?,?,?,?,?,?,?,?,?,? \
                                        );\
                                    `, [
                                        ...
                                        files[key].md5,
                                        files[key].batch,
                                        files[key].upload_time,
                                        files[key].extend,
                                        now,
                                        fac.fs.formatSize(speed)
                                    ], false);
