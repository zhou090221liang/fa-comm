    ...
    try {
        ...
        if (req.method.toUpperCase() == "POST") {
                        ...
                        if (err) {
                            ...
                            for (const key in files) {
                                if (files[key].name && files[key].size && files[key].path && files[key].type && files[key].lastModifiedDate && files[key].hash) {
                                    ...
                                    const exists = await sqlite3.all(`select * from upload where md5 = '${files[key].md5}' order by upload_time;`, null, false);
                                    if (exists && exists.length) {
                                        if (fs.existsSync(exists[0].path + exists[0].file_name)) {
                                            ...
                                            await sqlite3.run(`update upload set file_name = '${files[key].file_name}' where md5 = '${files[key].md5}'`, null, false);
                                        }
                                    }
                                    // const _result = await facomm_upload.insert(files[key]);
                                    const _result = await sqlite3.run(`\
                                        insert into upload(\
                                            id,size,form,origin_name,type,boundary,file_name,path,md5,batch,upload_time,extend\
                                        )values(\
                                            '${files[key].id}','${files[key].size}','${files[key].form}','${files[key].origin_name}',\
                                            '${files[key].type}','${files[key].boundary}','${files[key].file_name}',\
                                            '${files[key].path}','${files[key].md5}','${files[key].batch}',\
                                            '${files[key].upload_time}','${files[key].extend}'\
                                        );\
                                    `, null, false);
                                    // if (_result && _result.insertId) {
                                    if (_result && _result.changes) {
                                        result.push({
                                            id: files[key].id,
                                            size: files[key].size,
                                            form: files[key].form,
            ...
            if (req.path.replace('/', '').isGuid()) {
                ...
                let file_info = await sqlite3.get(`select * from upload where id = '${guid}';`, null, false);
                if (!file_info) {
                    //没有上传记录
                    ...
                    if (!fs.existsSync(file_info.path + file_info.file_name)) {
                        ...
                        if (req.query && req.query.action && (req.query.action == "download" || req.query.action == "down")) {
                            ...
                            res.setEncoding = 'utf-8';
                            if (file_info.type.startWith('video/') ||
                                file_info.type.startWith('audio/') ||
                                file_info.type.startWith('image/') ||
                                file_info.type.startWith('text/') ||
                                // file_info.type == 'application/log' ||
                                file_info.type == 'application/pdf') {
                                res.writeHead(200, { 'Content-Type': file_info.type });
                            ...
                            } else if (file_info.type == 'application/log') {
...
server.on('listening', async function () {
    // //创建数据表
    // await mysql.query(table, null, false);
    // //获取文件上传表
