...
function _insert(data, showstdout) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        for (let key in _data) {
            let _rpi = pri.find(item => item.COLUMN_NAME == key);
            if (_rpi) {
                if (_rpi.EXTRA != 'auto_increment') {
                    columns.push(`\`${key}\``);
                    values.push(`${key}`);
                }
            } else {
...
function _update(data, showstdout) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        for (let key in data) {
            let _rpi = pri.find(item => item.COLUMN_NAME == key);
            if (_rpi) {
                where.push(`${_rpi.COLUMN_NAME} = @${_rpi.COLUMN_NAME}@`);
            } else {
                if (_existsColumnByTable(self.schema, key)) {
                    update.push(`\`${key}\` = @${key}@`);
                }
