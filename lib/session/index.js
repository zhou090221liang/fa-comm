        ...
        super(async () => {
            ...
            this.get = async function (sessionid) {
                ...
                if (fs.existsSync(file + '.e')) {
                    ...
                    file = "";
                    console.warn("session可能已经过期或本来就不存在");
                }
                if (file) {
            ...
            /**
             * 延长session有效期
             * @param {String} sessionid 需要延长有效期的session
             * @param {Number} time 延长时间，单位秒
             * @returns
             */
            ...
            /**
             * 重置session有效期
             * @param {String} sessionid 需要重置有效期的的session
             * @param {Number} time 有效期，单位秒
             * @returns
             */
            this.setExpire = async function (sessionid, time) {
                await AutoRemove(this.instance.id.decrypt());
                if (!sessionid) {
                    throw new Error('sessionid不能为空');
                }
                ...
                if (fs.existsSync(file)) {
                    session = JSON.parse(fs.readFileSync(file).toString().decrypt());
                    if (session) {
                        session.expire = new Date(new Date().valueOf() + time * 1000).format('yyyy-MM-dd hh:mm:ss');
                        fs.unlinkSync(file);
                        fs.writeFileSync(file, JSON.stringify(session).encrypt());
                    }
                }
