    ...
    constructor(namespance = 'default') {
        super(async () => {
            ...
            this.create = async function (data, expire) {
                ...
                if (expire) {
                    fs.writeFileSync(filepath + this.instance.id.decrypt() + '/' + sessionid + '.e', JSON.stringify({
                        expire: new Date(new Date().valueOf() + expire * 1000).format('yyyy-MM-dd hh:mm:ss'),
                        data,
                        create_time: new Date().format('yyyy-MM-dd hh:mm:ss')
                    }).encrypt());
                } else {
                    fs.writeFileSync(filepath + this.instance.id.decrypt() + '/' + sessionid + '.ue', JSON.stringify({
                        data,
                        create_time: new Date().format('yyyy-MM-dd hh:mm:ss')
                    }).encrypt());
                }
            ...
            this.get = async function (sessionid) {
                ...
                if (fs.existsSync(file + '.e')) {
                    session = JSON.parse(fs.readFileSync(file + '.e').toString().decrypt());
                } else if (fs.existsSync(file + '.ue')) {
                    session = JSON.parse(fs.readFileSync(file + '.ue').toString().decrypt());
                }
                return session;
            };
            /**
