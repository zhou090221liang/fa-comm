...
module.exports = class {
     ...
     * 从图像或其他文件中提取ts文件
     * @static
     * @param {String} filepath 需要提取的实体文件
     * @param {Boolean} saveas 是否另存为文件
     * @returns JSON对象，包含原始文件信息及提取后的ts文件流
     */
    static extractTs(filepath, saveas = false) {
        const hexMap = {
            "8950": "png",
            "ffd8": "jpg",
            "4749": "gif",
        ...
        // const origin = hexMap[(buffer[0].toString(16) + buffer[1].toString(16))] || "unknow";
        // for (let i = 0; i < buffer.length; i++) {
        //     const hex1 = buffer[i].toString(16);
        //     const hex2 = buffer[i + 1].toString(16);
        //     if (hex1 + hex2 == '4740') {
        //         if (i == 0) {
        //             return { buffer, origin };
        //         } else {
        //             const buf = new Buffer(buffer.length - i);
        //             // buf.fill(buffer, i);
        //             buffer.copy(buf, 0, i);
        //             return { buffer: buf, origin };
        //         }
        //     }
        // }
        // return null;
        ...
        return new Promise(async function (resolve, reject) {
            try {
                ...
                if (saveas) {
                    ...
                    result.filepath = path.join(dirname, guid.v22 + "_" + basename);
