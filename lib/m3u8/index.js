...
const _progress = require('../comm/progress');
let request;
 ...
 * @returns
 */
const parse = async function (uri) {
    let m3u8Obj = [];
    //转换地址
    uri = _url.parse(uri);
    //获取内容
 ...
 */
const cache = async function (uri, filepath, procNum) {
    _fs.mkdirSync(filepath);
    const m3u8Obj = await parse(uri);
    if (m3u8Obj.length > 1) {
        //多个m3u8
        let m3u8FileText = ["#EXTM3U"];
        for (let index = 0; index < m3u8Obj.length; index++) {
            let _filepath = filepath + m3u8Obj[index].BANDWIDTH + '/';
            _fs.mkdirSync(_filepath);
            await cacheOnce(m3u8Obj[index], _filepath, procNum);
        ...
        //单个m3u8
        await cacheOnce(m3u8Obj[0], filepath, procNum);
    }
}

const cacheOnce = async function (m3u8, filepath, procNum) {
    const _progressDownloadBar = new _progress({
        title: '下载中',
        showLocation: true,
        showDuration: true,
    ...
    const _progressConvertBar = new _progress({
        title: '解密',
        showLocation: true,
        showDuration: true,
        showRemainingTime: true,
        ...
        if (j < t) {
            try {
                ...
                downLoad(filepath + fname, m3u8.list[j].URI).then(function () {
                    success++;
                    _progressDownloadBar.render(m3u8.list.length, success + error);
    ...
    for (let j = 0; j < m3u8.list.length; j++) {
        ...
        _progressConvertBar.render(m3u8.list.length, j + 1);
    }
    _progressConvertBar.clear();
