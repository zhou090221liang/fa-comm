...
const _cache = async function (filepath, procNum) {
    ...
    if (m3u8Obj.length > 1) {
        ...
        let index = 0;
        self._cacheOnceEventEmitter.on("download", function (data) {
            data.action = 'download';
            self.cacheTaskProcess.emit("process", {
                current: index + 1,
                total: m3u8Obj.length,
                detail: data
            });
        });
        self._cacheOnceEventEmitter.on("decrypt", function (data) {
            data.action = 'decrypt';
            self.cacheTaskProcess.emit("process", {
                current: index + 1,
                total: m3u8Obj.length,
...
const cacheOnce = async function (m3u8, filepath, procNum, parent) {
        ...
        if (j < t) {
            try {
                ...
                downLoad(filepath + fname, m3u8.list[j].URI).then(function () {
                    ...
                    parent._cacheOnceEventEmitter.emit("download", {
                        result: 'success',
    ...
    for (let j = 0; j < m3u8.list.length; j++) {
        ...
        parent._cacheOnceEventEmitter.emit("decrypt", {
            total: m3u8.list.length,
            now: j + 1
        });
    }
