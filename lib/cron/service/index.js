        ...
        task: cron.schedule(_schedule.schedule, async function () {
            ...
            try {
                ...
                if (!fs.existsSync(exec_file)) {
                    await sqlite3.run("update cron_instance set end_time = ?,err_info = ? where id = ?", [
                        new Date().Format(),
                        `可执行文件${exec_file}不存在`,
                        instance.id,
                    ]);
                    //自动运行的任务，没有运行，自动改回启动状态
                    await sqlite3.run("update cron_config set status = '1' where id = ?", [
                        instance.cron_id,
                    ]);
                }
                let args = [exec_file, JSON.stringify({ conf, id: _schedule.id }).encrypt()];
                if (process.execArgv.join('').indexOf('--inspect-brk') >= 0 || process.execArgv.join('').indexOf('--inspect') >= 0) {
                    const port = await fac.port.getRandomUnUsePort();
                    args.insert('--inspect-brk=127.0.0.1:' + port);
                }
                const command = `${process.execPath} ${args.join(' ')}`;
                console.info(`运行命令：${command}`);
                instance.stdout += `运行命令：${command} ${os.EOL}`;
                ...
                proc.stdout.on('data', async (data) => {
                    console.log(`【Task ${_schedule.id}】:${data.toString()}`.toInfo());
                    instance.stdout += data.toString();
                });
                proc.stderr.on('data', async (data) => {
                    console.error(`【Task ${_schedule.id}】:${data.toString()}`.toError());
                    instance.stderr += data.toString();
                });
                proc.on('close', async (code) => {
                    //自动运行的任务，运行结束，自动改回启动状态
...
const once = async function (id) {
    ...
    const task = await sqlite3.get('select * from cron_config where id = ?', [id]);
    if (!task) {
        return new fac.sdk.UnifiedStyleErrorMessage(`任务不存在`);
    }
    if (task.status != 0) {
        return new fac.sdk.UnifiedStyleErrorMessage(`只有停止状态的任务才能单次运行`);
    }
    ...
    try {
        ...
        instance.stdout = '';
        instance.stderr = '';
        const exec_file = path.join(task.exec_path, task.exec_file);
        if (!fs.existsSync(exec_file)) {
            await sqlite3.run("update cron_instance set end_time = ?,err_info = ? where id = ?", [
                new Date().Format(),
                `可执行文件${exec_file}不存在`,
            ...
            //没有运行的任务自动还原成停止状态
            await sqlite3.run("update cron_config set status = 0 where id = ?", [id]);
            return new fac.sdk.UnifiedStyleErrorMessage(`可执行文件${exec_file}不存在`);
        }
        let args = [exec_file, JSON.stringify({ conf, id }).encrypt()];
        if (process.execArgv.join('').indexOf('--inspect-brk') >= 0 || process.execArgv.join('').indexOf('--inspect') >= 0) {
            const port = await fac.port.getRandomUnUsePort();
            args.insert('--inspect-brk=127.0.0.1:' + port);
        }
        ...
        proc.stdout.on('data', async (data) => {
            console.log(`【Task ${id}】:${data.toString()}`.toInfo());
            instance.stdout += data.toString();
        });
        proc.stderr.on('data', async (data) => {
            console.error(`【Task ${id}】:${data.toString()}`.toError());
            instance.stderr += data.toString();
        });
        proc.on('close', async (code) => {
            //运行完成的任务自动还原成停止状态
